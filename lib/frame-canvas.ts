...
async function renderStrip4(
  ...
  const imgs = await Promise.all(
    photos.slice(0, 4).map((p) => (p ? loadImg(p) : Promise.resolve(null)))
  );
  for (let i = 0; i < 4; i++) {
...
async function renderStrip3H(
  ...
  ctx.font = `22px ${isNews ? pirata : bagel}`;
  ctx.textAlign = "center";
  ...
  const imgs = await Promise.all(
    photos.slice(0, 3).map((p) => (p ? loadImg(p) : Promise.resolve(null)))
  );
  for (let i = 0; i < 3; i++) {
  ...
  ctx.font = `9px ${mono}`;
  ctx.textAlign = "left";
  ctx.fillText("★ ONE WINK · ONE PRINT", PAD, y + 11);
  ctx.textAlign = "right";
  ctx.fillText("SPARKLE BOOTH · 2026", W - PAD, y + 11);
...
async function renderCard(
  ...
  if (ctx.roundRect) {
    ctx.beginPath();
    ctx.roundRect(1.5, 1.5, W - 3, H - 3, 12);
    ctx.stroke();
  } else {
  ...
  if (img) {
    ctx.save();
    ctx.beginPath();
    ctx.rect(PAD, PAD, PHOTO_W, PHOTO_H);
    ctx.clip();
    const ar = img.width / img.height,
      boxAr = PHOTO_W / PHOTO_H;
    let sx = 0,
      sy = 0,
      sw = img.width,
      sh = img.height;
...
export async function exportFrameToPng(
  ...
  switch (layoutId) {
    case "strip4":
      ...
      break;
    case "trip3h":
      canvas = await renderStrip3H(photos, s.palette, styleId, meta);
      break;
    case "trip3v":
      canvas = await renderStrip3V(photos, s.palette, styleId, meta);
      break;
    case "card1":
      canvas = await renderCard(photos, s.palette, meta);
      break;
    default:
      canvas = await renderStrip4(photos, s.palette, styleId, meta);
  }
